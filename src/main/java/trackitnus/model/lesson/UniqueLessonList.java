...
import trackitnus.model.lesson.exceptions.DuplicateLessonException;
import trackitnus.model.lesson.exceptions.LessonNotFoundException;

/**
 * A list of lessons that enforces uniqueness between its elements and does not allow nulls.
 * A lesson is considered unique by comparing using {@code Lesson#isSameLesson(Lesson)}. As such,
 * adding and updating of lessons uses Lesson#isSameLesson(Lesson) for equality so as to ensure that
 * the lesson being added or updated is unique in terms of identity in the UniqueLessonList.
...
public class UniqueLessonList implements Iterable<Lesson> {
    ...
    public void setLessons(List<Lesson> lessons) {
        CollectionUtil.requireAllNonNull(lessons);
